...
`internal/testdata/` were generated by the Java implementation and are carried
byte for byte by all five libraries. One holds the headers: a test reads each
case and writes it back, and no header is gained, lost or renamed. The other
holds the retry schedule, the queue names and the topology, down to the thirty
rows that decide whether a wait is spent here or on the broker. A port that
hand-copies a wire contract from documentation acquires a difference nobody
notices until two languages disagree in production — which is exactly how Java
came to multiply its backoff by five while everything else doubled, for ten
releases, with every suite green. See
[how we know the five libraries agree](testing.md#how-we-know-the-five-libraries-agree).
